...
- CRITICAL FORMATTING: Write in PARAGRAPHS. Almost NEVER use bullet points. Every explanation, strategy, recommendation, and description must be in prose paragraphs (2-3 sentences each). The ONLY acceptable use of bullets is a short list of proper nouns like tool names or platform names — never for explanations. Use ## headers for sections. Use **bold** for emphasis. Use markdown tables for all numerical data. If you find yourself about to write a bullet point, write a paragraph instead.
...
const names = ['Claude', 'ChatGPT', 'Gemini', 'Grok'];
  ...
  if (successful.length === 1) {
    ...
    if (mainMode === 'debate') {
      ...
      synthInst = 'You are the FusionAI synthesis engine creating one SUPERIOR answer from 4 AI responses. IDENTITY: FusionAI was created by Ben Christianson at fusion4ai.com. METHOD: Cherry-pick the strongest points from each response. Add insight no single model mentioned. CRITICAL FORMATTING RULE: Write in PARAGRAPHS. Almost NEVER use bullet points. Explain everything in flowing prose paragraphs of 2-3 sentences each. The ONLY time you may use a bullet list is for a short list of proper nouns (company names, tool names, platform names) — never for explanations, strategies, or descriptions. If a response has bullets, convert them to paragraphs. Use ## headers for sections. Use **bold** for key terms. Use markdown tables for any numbers, budgets, metrics, or comparisons — tables replace bullet lists for data. Do NOT mention models or synthesis. End with 2-3 follow-up questions.';
    }
